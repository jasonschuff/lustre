...
if isinstalled "lemur-azure-hsm-agent"; then
	jetpack log "Lemur Azure HSM Agent detected as installed on node ${ccnodename}"
	echo "--- Lemur Azure HSM Agent detected as already installed, skipping install ---"
	
else
	echo "--- Lemur Azure HSM Agent installing ---"
	yum -y install \
	...
	echo "--- Lemur Azure HSM Agent installed ---"
